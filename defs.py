...
async def charge_resp(result ):
    ...
    try:
        ...
        if (
            '"payment_success": true' in result 
            or '"success":true' in result
            # or '"payment_success": false' not in result
            or 'succeeded' in result
            or '"result": "success"' in result  or "Membership Confirmed" in result or "Order Completed" in result
            ):
            response = "Charged 1$ 🔥"
        ...
        elif '"Gateway Rejected: avs_and_cvv"'in result or "card has insufficient funds." in result:
            response = "Gateway Rejected: avs_and_cvv ❎"
        
        elif "The provided card is expired, please use an alternate card or other form of payment" in result:
        ...
        elif "stripe_3ds2_fingerprint" in result or '"requiresAction":true,' in result:
            response = "3D Challenge Required ❎"
            ...
            response = "Card Number Is Incorrect 🚫"
